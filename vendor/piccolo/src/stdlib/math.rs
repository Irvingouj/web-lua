        ...
        callback(
            ...
            move |_, (u, l): (Option<u64>, Option<u64>)| {
                ...
                match (u, l) {
                    ...
                    (Some(high), Some(low)) => {
                        // SmallRng seed size varies by platform:
                        // - wasm32: 16 bytes (ChaCha8)
                        // - native: 32 bytes (ChaCha8)
                        // Use seed_from_u64 which works everywhere.
                        let combined = (high << 32) | (low & 0xFFFFFFFF);
                        *rng.borrow_mut() = SmallRng::seed_from_u64(combined);
                        Some(())
                    }
                    _ => None,
                }
