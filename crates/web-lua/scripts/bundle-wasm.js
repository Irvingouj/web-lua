...
const wasmPath = path.join(pkgDir, 'web_lua_bg.wasm');
const jsPath = path.join(pkgDir, 'web_lua.js');
const dtsPath = path.join(pkgDir, 'web_lua.d.ts');
...
js = js.replace(
    /if \(module_or_path === undefined\) \{\s*module_or_path = new URL\('web_lua_bg\.wasm', import\.meta\.url\);\s*\}/,
...
if (fs.existsSync(jsDir)) {
    fs.copyFileSync(jsPath, path.join(jsDir, 'web_lua.js'));
    if (fs.existsSync(dtsPath)) {
        fs.copyFileSync(dtsPath, path.join(jsDir, 'web_lua.d.ts'));
    }
